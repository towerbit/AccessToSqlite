...
namespace AccessToSqlite.Core
{
    public class Export
    {
        ...
        public ExportProgressChangedEventHandler ExportProgressChanged;

        public Export(AccessExportOptions options)
        {
            _options = options;
            _sqliteProvider = new SqliteProvider(options.SQLiteFileName, options.SQlitePassword);
        }
        ...
        public ExportResult Execute()
        {
            ...
            try
            {
                ...
                foreach (var table in ds.Tables.Cast<DataTable>())
                {
                    ...
                    ExportProgressChanged?.Invoke(this, new ExportProgressChangedEventArgs() { TableName = table.TableName });
                    InsertData(table);
                }
            }
